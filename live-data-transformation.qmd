...
```{r}
sanitation <- read_csv("data/jmp-washdata-indicators-safely-managed-sml.csv")
...
# Our turn (1): filter, arrange, select

We start with three verbs together: `filter()`, `arrange()`, and `select()`.
...
# Our turn (2): the remaining verbs

We work through the remaining verbs: `rename()`, `relocate()`, `mutate()` (and factors), and `group_by()` & `summarise()`.
